...
#引用多的，而且引入的都是需要看的那几个文件，肯定后看了
from rpn_msr.anchor_target_layer_tf import anchor_target_layer as anchor_target_layer_py
from rpn_msr.proposal_target_layer_tf import proposal_target_layer as proposal_ta rget_layer_py


#主要看的是1W+一呆飞天，两个Train看得是4W,后期第一个train好像看的是1w/4w的部分，看没看忘记了
...
class Network(object):
    ...
    def validate_padding(self, padding):
        assert padding in ('SAME', 'VALID')#这个怎么还要验证吗
    ...
    def conv(self, input, k_h, k_w, c_o, s_h, s_w, name, relu=True, padding=DEFAULT_PADDING, group=1, trainable=True):
        ...
        with tf.variable_scope(name) as scope:#函数调用模块化是主流，同时对于各层的参数也是建立范围，包括范围和参数本身也是有通用的部分，也卸载函数内
            #重中之重：对于变量的定义和conv2d按道理是上成函数的。使用天池组合，同时，加上

            init_weights = tf.truncated_normal_initializer(0.0, stddev=0.01)#天池组合
...
#！！！～调用函数和参数总是一起出入，而且调用就是直接喊出小函的名字，无论沧海桑田，这个是不变的i ,k对应的传参,
            else:#在这里group表示均匀切割的数量
    ...
    def roi_pool(self, input, pooled_height, pooled_width, spatial_scale, name):
        # only use the first input，看不懂两行赋值，看不懂input[0][0]的含义
        if isinstance(input[0], tuple):#内置函数isinstance顾名思义,判断变量是否是后面的类型或者python独有的数据类型,这个里边怎么直接来了个实参，内置函数也不能这样吧
            input[0] = input[0][0]
    ...
    def proposal_layer(self, input, _feat_stride, anchor_scales, cfg_key, name):
        if isinstance(input[0], tuple):
            input[0] = input[0][0]                                                                                                       #all,5列---来自于知乎收藏
    ...
    def anchor_target_layer(self, input, _feat_stride, anchor_scales, name):
        ...
        with tf.variable_scope(name) as scope:
            ...
            rpn_labels,rpn_bbox_targets,rpn_bbox_inside_weights,rpn_bbox_outside_weights = tf.py_func(anchor_target_layer_py,
                    [input[0],input[1],input[2],input[3], _feat_stride, anchor_scales],[tf.float32,tf.float32,tf.float32,tf.float32])
#列表，数组 convert to tensor 如果原来就是数组，那么就是type后发现很明显类型变了
...
它是在convert后面使用，同时两者都是小打小闹，典型代表就是图片转换成tf.string ,各种label转换成tf.int32的数据类型
例如：
    ...
    def reshape_layer(self, input, d,name):
        ...
        if name == 'rpn_cls_prob_reshape':
             return tf.transpose(tf.reshape(tf.transpose(input,[0,3,1,2]),[input_shape[0],
                    int(d),tf.cast(tf.cast(input_shape[1],tf.float32)/tf.cast(d,tf.float32)*tf.cast(input_shape[3],tf.float32),tf.int32),
