...
#～经典
            orig_0 = weights.eval()   #be equal to "sess = tf.Session()  && sess.run(weights考虑到好算wx所以也是tensor的形式) sess.run可以是tensor参数，也可以是列表，tunple等可遍历的
            orig_1 = biases.eval()

            # scale and shift with bbox reg unnormalization; then save snapshot
            weights_shape = weights.get_shape().as_list()

  #这个run的是啥我知道，但是这个东西从哪个模块导入的

#～ sess.run（arg,feed_dict)可以通俗理解为在XXXXXX情况下/数据上的arg)
    ...
    def _modified_smooth_l1(self, sigma, bbox_pred, bbox_targets, bbox_inside_weights, bbox_outside_weights):
        """
            ...
            SmoothL1(x) = 0.5 * (sigma * x)^2,    if |x| < 1 / sigma^2
                          |x| - 0.5 / sigma^2,    otherwise     公式本身简单，只需要先出tf.XXXX（加减乘，绝对值，除法用符号代替）
        """
        sigma2 = sigma * sigma
        ...
        for iter in range(max_iters):
            ...
            if (iter+1) % (cfg.TRAIN.DISPLAY) == 0:
                print('iter: %d / %d, total loss: %.4f, rpn_loss_cls: %.4f, rpn_loss_box: %.4f, loss_cls: %.4f, loss_box: %.4f, lr: %f'%\
                        (iter+1, max_iters, rpn_loss_cls_value + rpn_loss_box_value + loss_cls_value + loss_box_value ,rpn_loss_cls_value,
                         rpn_loss_box_value,loss_cls_value, loss_box_value, lr.eval()))
...
def train_net(network, imdb, roidb, output_dir, pretrained_model=None, max_iters=40000):
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
        sw = SolverWrapper(sess, saver, network, imdb, roidb, output_dir, pretrained_model=pretrained_model)#end
