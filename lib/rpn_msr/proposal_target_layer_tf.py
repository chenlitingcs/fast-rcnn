...
def proposal_target_layer(rpn_rois, gt_boxes,_num_classes):
    ...
    #～ 超级管用 我们可以直接用XXX.reshape( , )对数组，矩阵进行形状的改变和获取形状的shape[0],shape[1],shape[1:],shape[-2:]也很常用
...
def _sample_rois(all_rois, gt_boxes, fg_rois_per_image, rois_per_image, num_classes):
    ...
    examples.生成包含前景和背景的RoI(特征图上的框）的随机样本
    """
    # overlaps: (rois x gt_boxes)
#bbox_overlaps： 返回:overlaps 即 (len(inds_inside)* len(gt_boxes)
#overlaps其实就是每个anchors与我们手工标注每个的gt_boxes重叠的面积
